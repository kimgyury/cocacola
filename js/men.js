...
$(function(){//jQB2/////////////////////////////////////
	...
	//신상품 li에 마우스 오버시 멈춤, 정보보이기, 아웃시 다시 실행
	$(".flowImg li").hover(
		function(){//over
			clearInterval(acall);//진행멈춤!
			/// 상품정보 보이기 ///
			var cls=$(this).attr("class");//클래스명 읽어오기!
			var info=sinsang[cls];//셋팅된 객체변수의 신상정보 가져오기!
//			console.log(info);
			
			//// 상품 정보 박스를 만들고 나타나게 하기 ///
			$(this).append('<div class="ibox"></div>');
			$(".ibox").html(info.replace(/\^/g,"<br>"))// 특수문자는 정규표현식에서 역슬래쉬(또는 원화표시)를 사용함! 
			.animate({top:"105%",opacity:1},300,"easeOutCirc");
	...
	$(".flowImg>li").click(function(){
		var gcode=$(this).attr("class");//클래스명이 상품코드임!
...
//	console.log(iw);
	...
	fnum--;//1씩감소
	// 이미지 하나의 width 크기 만큼 left값이 변경되면 이때 나간 이미지를 잘라서 맨뒤로 보내고 css의 left값과 fnum값을 0으로 초기화한다!
	if(fnum<=-iw){
